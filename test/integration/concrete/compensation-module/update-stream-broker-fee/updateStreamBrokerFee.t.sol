...
contract UpdateStreamBrokerFee_Integration_Concret_Test is Integration_Test {
    ...
    function test_UpdateStreamBrokerFee() external whenCallerBrokerAdmin {
        ...
        vm.expectEmit();
        emit IFlowStreamManager.BrokerFeeUpdated({ oldFee: ud(0), newFee: newBrokerFee });

        // Run the test
        compensationModule.updateStreamBrokerFee(newBrokerFee);

        // Assert the actual and expected broker fee
        UD60x18 actualBrokerFee = compensationModule.broker().fee;
