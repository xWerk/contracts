...
// Fixed Extensions
import "@thirdweb/contracts/extension/Multicall.sol";
...
import "@thirdweb/contracts/external-deps/openzeppelin/utils/cryptography/ECDSA.sol";

import "@thirdweb/contracts/prebuilts/account/interface/IAccountCore.sol";
...
/// Note: Fork of the thirdweb `AccountCore.sol` contract which renames the `initialize` method
/// to `__AccountCore_init` to adhere to the UUPS pattern across the Werk codebase.
/// This implementation also removes the `_salt` parameter from `onSignerAdded` and `onSignerRemoved` methods
